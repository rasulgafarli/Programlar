...
# ==============================================================================
# YENİ ƏLAVƏ EDİLMİŞ TƏK HƏRF NƏTİCƏ PƏNCƏRƏSİ
# ==============================================================================
class SingleLetterInfoWindow(QDialog):
    ...
    def __init__(self, letter_to_analyze, parent=None):
        ...
        try:
            ...
            # Bu halın baş verməməsi üçün əsas pəncərədə yoxlama var, ancaq ehtiyat üçün
        ...
        headers = ["Daxil edilən hərf:", "Əlifbadakı yeri:", "Əvvəlki hərf:", "Sonrakı hərf:"]
        values = [letter_to_analyze.upper(), str(position), prev_letter, next_letter]
...
# ==============================================================================
# YENİ NƏTİCƏ PƏNCƏRƏLƏRİ
# ==============================================================================
# Dodaqlanan və Dodaqlanmayan saitlərin nəticə pəncərəsi
...
# Kar və Cingiltili samitlərin nəticə pəncərəsi
...
class VowelConsonantResultsWindow(QDialog):
    def __init__(self, sentence_to_analyze, parent=None):
        ...
        for widget in (
                self.vowels_header_label, self.vowels_list_label, self.consonants_header_label,
                self.consonants_list_label):
            widget.hide()
        self.close_button = QPushButton("Bağla")
        self.close_button.setObjectName("closeButton")
...
class MainWindow(QWidget):
    ...
    # DƏYİŞDİRİLMİŞ VƏ YENİ MƏNTİQ ƏLAVƏ EDİLMİŞ METOD
    def validate_and_proceed(self):
        ...
        # 1. Mətnin boş olub-olmadığını yoxla
        ...
        # 2. YENİ ŞƏRT: Mətnin tək bir hərf olub-olmadığını yoxla
        elif len(stripped_text) == 1 and stripped_text.isalpha() and self.is_valid_sentence(stripped_text):
            self.error_label.hide()
            self.text_edit_container.setProperty("hasError", False)
            # Tək hərf üçün xüsusi pəncərəni aç
            dialog = SingleLetterInfoWindow(stripped_text, self)
            dialog.exec()

        # 3. Mətndə icazə verilməyən simvol olub-olmadığını yoxla
        ...
        # 4. Əgər yuxarıdakı hallar deyilsə, standart axını davam etdir
        else:
