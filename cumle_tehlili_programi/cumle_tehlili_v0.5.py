# --- Söz və Hərf Sayğacı Proqramı (Animasiyalı Düymələr ilə) ---
...
from PyQt6.QtCore import Qt, QTimer  # <-- YENİ: QTimer əlavə edildi
    ...
    def __init__(self, sentence_to_analyze, parent=None):
        ...
        self.setStyleSheet("""
            ...
            QLabel {
                ...
                border: 2px solid #F39C12; border-radius: 12px; padding: 15px;
            }
            QPushButton {
    ...
    def __init__(self, sentence_to_analyze, parent=None):
        ...
        self.setStyleSheet("""
            ...
            QLabel {
                ...
                border: 2px solid #16A085; border-radius: 12px; padding: 15px;
            }
            QPushButton {
                font: bold 12px; color: white; padding: 8px 15px;
                border-radius: 16px; border: none; background-color: #E74C3C;
    ...
    def __init__(self, sentence_to_analyze, parent=None):
        ...
        self.setStyleSheet("""
            ...
            QLabel#vowelConsonantLabel {
                ...
                border: 2px solid #3498DB; border-radius: 12px; padding: 15px;
            }
            QPushButton {
                font: bold 12px; color: white; padding: 8px 15px;
...
class ResultsWindow(QDialog):
    def __init__(self, sentence_to_analyze, parent=None):
        ...
        self.setStyleSheet("""
            QDialog { background-color: #F2F2FE; }
            QLabel#resultLabel {
                color: #341F4E; background-color: #E8E6F8;
                border: 2px solid #8E44AD; border-radius: 12px; padding: 15px;
            }
            QPushButton {
                font: bold 12px; color: white; padding: 8px 15px;
                border-radius: 16px; border: none;
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        grid_layout = QGridLayout(text_edit_container)
        ...
        # --- YENİ: Ardıcıl göstərmək üçün düymələri bir siyahıya yığırıq ---
        self.result_buttons = [
        ...
        main_layout.addWidget(text_edit_container)
        main_layout.addWidget(self.error_label)

        button_layout_row1 = QHBoxLayout()
        button_layout_row1.addWidget(self.result_buttons[0])  # Sait/Samit
        button_layout_row1.addWidget(self.result_buttons[1])  # Söz/hərf sayı

        button_layout_row2 = QHBoxLayout()
        button_layout_row2.addWidget(self.result_buttons[2])  # Qalın/İncə
        button_layout_row2.addWidget(self.result_buttons[3])  # Açıq/Qapalı

        self.shadow_effects = [QGraphicsDropShadowEffect(blurRadius=15, xOffset=0, yOffset=2, color=QColor(0, 0, 0, 80))
                               for _ in range(6)]
    ...
    def apply_styles(self):
        self.setStyleSheet("""
            ...
            QTextEdit {
                background-color: white; border: 1px solid #D2B4DE; padding: 8px 35px 8px 10px;
                font-size: 16px; border-radius: 18px;
            }
            QTextEdit:focus { border: 2px solid #8E44AD; }
            QTextEdit[hasError="true"] { border: 2px solid #E74C3C; }
    ...
    # --- YENİ: Düymələri ardıcıl göstərən animasiya funksiyası ---
    def show_buttons_sequentially(self, index=0):
        ...
        button_to_show.show()
        self.adjustSize()  # Hər düymə göründükdən sonra pəncərəni yenidən ölç

        # Növbəti düyməni 100ms sonra göstər
    ...
    def validate_and_proceed(self):
        ...
        is_valid = self.is_valid_sentence(text) and text.strip()

        # --- DƏYİŞİKLİK: Animasiyanı idarə etmək üçün məntiq ---
        if is_valid:
            self.sentence_to_pass = text
            self.error_label.hide()
            self.input_box.setProperty("hasError", False)
            # Düymələri birbaşa göstərmək yerinə, animasiyanı başlat
            self.show_buttons_sequentially()
        else:
            self.sentence_to_pass = ""
            # Bütün nəticə düymələrini gizlət
            for button in self.result_buttons:
                button.hide()
            self.adjustSize()  # Pəncərəni kiçilt
